...
/// <reference types="@ts-defold/types" />\n`;
		...
		const isAllUppercase = function (str) {
			return /^[A-Z_]+$/.test(str);
		};
		// Check type of API entry
		const isApiTable = function (entry) {
			return (
		...
		// Check type of API entry
		const isApiFunc = function (entry) {
			return (
				(typeof entry.type === 'string' &&
		...
		// Sanitizes name
		const getName = function (name, isParam) {
		...
		const getType = function (type, context) {
		...
		const sanitizeForComment = function (str) {
			return str.replace(/\*\//g, '*\\/');
		};
		// Transforms and sanitizes descriptions
		const getComments = function (entry) {
		...
		const getReturnComments = function (returnObj, newDesc) {
			let returnType = '';
			let comments = '';
		...
		const getParamComments = function (parameters, newDesc) {
		...
		const getParamFields = function (fields, newDesc) {
			fields.forEach((field) => {
				newDesc += ` ${JSON.stringify(field)}`;
			});
			return newDesc;
		};
		const getExampleComments = function (examples, newDesc) {
		...
		const generateTableDefinition = function (entry, details, start = false) {
		...
		const generateFunctionDefinition = function (entry, isParam) {
		...
		const getParameterDefinition = function (param) {
		...
		const getReturnType = function (returnObj) {
			if (!returnObj) {
				return 'void';
			}
			if (Array.isArray(returnObj)) {
		...
		const generateEntryDefinition = function (entry) {
			const name = entry.name
				? getName(entry.name, false)
				: DEFAULT_NAME_IF_BLANK;
		...
		const generateTypeScriptDefinitions = function (api, details) {
			let definitions = '';
			const namespaces = {};
			api.forEach((entry) => {
