...
function parseTypeDefinition(entry: PatchEntry, root: boolean) {
...
function parseTable(
	...
	if (entry.members && Array.isArray(entry.members)) {
		...
		if (!isApiFunc(entry)) {
			(entry as ScriptApiTable).members!.sort(alphabetizeApi);
		}
...
function getParams(param: ScriptApiParameter): string {
		...
		if (param.parameters) {
			...
			param.parameters.map((subparam) => {
				if (subparam && subparam.name) {
					let subParamType = getType(
						subparam.type,
						'param',
						subparam.useExactType,
					);
					const fOptional = subparam.optional ? '?' : '';

					if (
						subParamType === KNOWN_TYPES['TABLE'] &&
...
function getReturnType(
	returnObj:
		...
		| ScriptApiTable
		| ScriptApiFunction
		| Array<ScriptApiEntry | ScriptApiTable | ScriptApiFunction>
		| undefined,
): string {
	if (!returnObj) {
		return 'void';
...
function parseGeneric(entry: ScriptApiEntry, root: boolean): string {
	const declaration = getDeclarationKeyword(root);
	const override = entry.name ? getExportOverride(entry.name, false) : '';
