...
function recursiveMerge(entry, patch) {
    if (patch.desc) {
    ...
    if (patch.returns) {
        ...
        if (Array.isArray(patch.returns) &&
            Array.isArray(entry.returns)) {
            entry.returns.forEach((old, oldIndex) => {
                patch.returns.forEach((upd, updIndex) => {
                    if (oldIndex === updIndex) {
                        if (old.desc && !upd.desc) {
                            patch.returns[updIndex].desc = entry.returns[updIndex].desc;
                        }
                    }
                });
            });
        }
        entry.returns = patch.returns;
    }
    ...
    if (patch.members) {
        if (entry.members) {
            entry.members.map((old) => {
                patch.members.map((upd) => {
                    if (old.name === upd.name) {
                        old = recursiveMerge(old, upd);
                    }
                });
                return old;
            });
            ...
            entry.members = patch.members;
        }
    }
    // Recursively handle parameter tables
    if (patch.parameters) {
        if (entry.parameters) {
            entry.parameters.map((old) => {
            ...
            entry.parameters = patch.parameters;
        }
    }
    // Recursively handle fields tables
    if (patch.fields) {
        if (entry.fields) {
            entry.fields.map((old) => {
 ...
 * @returns
 */
export function applyGlobalPatches(api, key) {
    if (prePatches[key]) {
