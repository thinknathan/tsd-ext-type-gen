 ...
 */
function isTypeDef(entry) {
    return entry.type === 'typeDef' && !!entry.typeDef;
}
...
function getName(name, isParam) {
    ...
    if (!isParam && modifiedName !== name) {
        console.warn(`Modifying invalid ${typeof name} "${name}" to "${modifiedName}"`);
    }
 ...
 */
function getDeclarationKeyword(root) {
    if (root) {
        return 'declare';
    }
    else {
...
export { isAllUppercase, isApiTable, isApiFunc, isNameInvalid, isTypeDef, sanitizeForComment, getName, getDeclarationKeyword, getExportOverride, alphabetizeApi, };
