...
function getParamComments(parameters, newDesc) {
    parameters.forEach((param) => {
        ...
        if (name) {
            ...
            if (param.members &&
                Array.isArray(param.members)) {
                param.members.forEach((subparam) => {
                    newDesc += getSubParamComments(subparam);
                });
            }
            if (param.parameters &&
                Array.isArray(param.parameters)) {
                param.parameters.forEach((subparam) => {
                    newDesc += getSubParamComments(subparam);
                });
            }
        }
...
function getSubParamComments(subparam) {
...
function getExampleComments(examples) {
    let exampleText = '';
